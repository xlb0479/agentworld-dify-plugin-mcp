...
class McpTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        arguments_json = tool_parameters.get("tool_purposes", "")
        if not arguments_json:
            raise ValueError("Please fill in the tool_purposes")
        try:
            arguments = {"intents": json.loads(arguments_json)}
        except json.JSONDecodeError as e:
            raise ValueError(f"tool_purposes must be a valid JSON string: {e}")
